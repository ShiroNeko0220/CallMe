...
export default function StatistiquesView({ role }) {
  ...
  const nomMembre = (id) => {
    ...
    return m ? `${m.prenom} ${m.nom}` : `Élève #${id}`
  }

  useEffect(() => {
    // Charger référentiels
    api.utilisateurs.lister(role)
        .then(r => setMembres(r.data.filter(u => String(u.role).toUpperCase() === 'MEMBRE')))
        .catch(() => {})
  ...
  return (
      <div>
        ...
        {/* ── Bloc 4 : Parcours d’un élève ────────────────────────────── */}
        <Card title={<span className="flex items-center gap-2"><TrendingUp size={15} className="text-blue-500" /> Parcours d’un élève</span>}>
