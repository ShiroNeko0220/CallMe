...
export default function BadgesView({ role }) {
  ...
  const associer = async () => {
    ...
    try {
      ...
      if (status === 404) {
        setAlert({ type: 'error', message: 'Badge ou membre non existant. Vérifiez les numéros saisis.' })
      } else if (status === 409) {
        ...
        {role === 'SECRETAIRE' && (
            <Card title="Associer un badge à un membre">
              <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-3 items-end">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Numéro du badge</label>
                  <input
                      value={assocForm.idBadge}
